...
export namespace AuthenticationCoreKyselyRegister {
    function mapFetchedUser(row: Record<string, unknown> | undefined): authenticationCoreLib.FetchedUser | undefined {
        if (!row) {
            return undefined;
        }
    ...
    export async function register(
        ...
        mailTransportConfig?: Parameters<typeof authenticationCoreLib.AuthenticationCoreRegister.register>[5],
        hashOptions?: Parameters<typeof authenticationCoreLib.AuthenticationCoreRegister.register>[6],
        ...
        const mailExistsRoutine = async (mail: string): Promise<boolean> => {
            ...
            const result = await transactionConnection.query(compiledQuery.sql, compiledQuery.parameters);
        ...
        const dataProcessing = async (
            ...
            const result = await transactionConnection.query(compiledQuery.sql, compiledQuery.parameters);
            return mapFetchedUser(result.rows?.[0] as Record<string, unknown> | undefined);
        };

        try {
            await transactionConnection.query("BEGIN");

            const registeredUser = await authenticationCoreLib.AuthenticationCoreRegister.register(
