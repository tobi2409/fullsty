...
export namespace AuthenticationCoreKyselyLogin {
    ...
    export async function login(
        ...
        jwtKey: Parameters<typeof authenticationCoreLib.AuthenticationCoreLogin.login>[3],
        dbConnection: DbConnection,
        tableName: string = DEFAULT_TABLE_NAME,
        columns: UserColumn = DEFAULT_USER_COLUMN,
        jwtOptions: Parameters<typeof authenticationCoreLib.AuthenticationCoreLogin.login>[4] = {}
    ): Promise<string> {
        try {
            ...
            const fetchedUser = mapFetchedUser(result.rows?.[0] as Record<string, unknown> | undefined);
            return authenticationCoreLib.AuthenticationCoreLogin.login(typedMail, typedPassword, fetchedUser, jwtKey, jwtOptions);
        } catch (error) {
            throw error;
        }
    }
