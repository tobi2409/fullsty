...
const data = TemplateEngine.reactive({
    loginSection: {
        ...
        okClick: async () => {
            try {
                const token = await Server.loginUser(data.loginSection.email, data.loginSection.password)
    ...
    registerSection: {
        ...
        okClick: async () => {
            try {
                const fetchedUser = await Server.registerUser(data.registerSection.email, data.registerSection.password)
    ...
    currentUserSection: {
        ...
        okClick: async () => {
            try {
                const uuid = await Server.getCurrentUser(data.currentUserSection.token)
